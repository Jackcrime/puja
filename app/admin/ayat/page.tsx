...
// ─── Helpers: convert Firestore hierarchical ↔ flat admin list ────────────────
function categoriesToFlat(cats: { category: string; verses: { label: string; reference: string; text: string }[] }[]): Verse[] {
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function AdminAyat() {
  ...
  // Sync dari Firestore ke flat list saat data masuk
  useEffect(() => {
    if (!loading) setVerses(categoriesToFlat(categoryData));
  }, [loading, categoryData]);

  // Simpan ke Firestore (convert kembali ke hierarchical)
  ...
  return (
    <AdminGuard>
      <AdminLayout title="Ayat Emas">
        {/* Status bar */}
        <div className="mb-4 flex items-center gap-3">
          <p className="text-sm text-muted-foreground">
            {loading ? "Memuat dari Firestore..." : `${verses.length} ayat tersimpan`}
          </p>
          {saving && (
